...
class FakeLLMProvider:
    ...
    def generate_code(self, prompt: str):
        return (
            ...
            "        title = Text('Goal').to_edge(UP)\n"
            "        board = Rectangle(width=8, height=4).set_stroke(BLUE, width=2)\n"
            "        eq_1 = Text('2x + 3 = 11', font_size=40).move_to(board.get_center() + UP*0.8)\n"
            "        eq_2 = Text('x = 4', font_size=40).move_to(board.get_center() + UP*0.8)\n"
            "        explain = Text('Show equation and transform line by line.', font_size=24).next_to(board, DOWN)\n"
            "        scene_1_group.add(title, board, eq_1, explain)\n"
            "        self.play(FadeIn(scene_1_group), run_time=0.8)\n"
            "        self.play(Indicate(eq_1), run_time=0.3)\n"
            "        self.play(ReplacementTransform(eq_1, eq_2), run_time=0.6)\n"
            "        scene_1_group.add(eq_2)\n"
            "        self.wait(0.3)\n"
            "        active_group = scene_1_group\n"
            "        # SCENE_END:1\n"
            "        self.play(FadeOut(active_group), run_time=0.2)\n"
        )
